...
if(isset($_POST["search"])){
    // バー名が入力されていて評価が入力されていない時
    if(isset($_POST["barName"]) && empty($_POST["rating"])){
        // データ取得
        $barName = $_POST["barName"];
        $rating = "";

    // 評価が入力されていてバー名が入力されていない時
    }else if(empty($_POST["barName"]) && isset($_POST["rating"])){
        $barName = "";
        $rating = $_POST["rating"];

    // バー名も評価も入力されている時
    }else if(isset($_POST["barName"]) && isset($_POST["rating"])){
        $barName = $_POST["barName"];
        $rating = $_POST["rating"];
    }

    $select = "SELECT * FROM gs_bar_memo WHERE barName LIKE '%{$barName}%' AND rating LIKE '%{$rating}%'";
    ...
    if($status == false){
      ...
      //Selectデータの数だけ自動でループしてくれる
        while( $result = $stmt->fetch(PDO::FETCH_ASSOC)){
            ...
            $view .= "<td><a href='".$result['mapUrl']."'>Google map</a></td>";
...
<body>
    ...
    <!-- 検索欄 -->
    <form class="mx-auto" action="./search.php" method="POST" name="" id="mainForm" style="width:90vw;">
        <div class="input-group mb-3">
            <span class="input-group-text" id="inputGroup-sizing-default">Bar</span>
            <input type="text" class="form-control" name="barName">
        </div>
        <div class="input-group mb-3">
        ...
        <!-- <div class="row">
            ...
            <div style="width: 20px;text-align:center;padding:0">〜</div>
            <div class="col">
                <input type="date" class="form-control">
            </div>
        </div> -->
            
        <input type="submit" name="search" value="検索" class="mx-auto mb-3" style="display:block;">
    </form>
